...
for (x in 1:max(unlist(RGB_band_df[,4])))
{
  ...
  for(i in 2:15)
  {
    ...
    if((kmeans(y[,1:3],centers=i)$tot.withinss/kmeans(y[,1:3],centers=i)$betweenss)<=0.05)
    {
      y$cluster<-kmeans(y[,1:3],centers=i)$clust
      break()
    } 
    if(i==15)
    {
      y$cluster<-kmeans(y[,1:3],centers=1)$clust
      break()
    }
  }
  for(i in 1:length(unique((y$cluster))))
  {
    if(i>0)
    {
